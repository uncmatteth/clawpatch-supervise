...
From a target repository, bare `clawpatch-supervise` uses the current branch, pushes each verified repair, and processes the queue without a start-mode prompt. It resumes any exact stopped checkpoint automatically, preserves an open queue, revalidates every retained uncertain finding, and starts a fresh full review automatically only when both the existing queue and project source are proven clean. Interrupted progress recorded for a different branch is retired automatically, without switching branches or changing source, so stale release-sweep state cannot block supervision on the current branch. When a stopped checkpoint and the current source changes no longer match, the command snapshots every changed source path in a local recovery commit and receipt, restores current HEAD, retires the stale checkpoint, and continues without waiting for an operator. If source changes remain after recovery without any checkpoint owning them, the external supervisor preserves that complete tree under the same local-only recovery contract, restores current HEAD, keeps `.clawpatch`, and continues instead of waiting forever. Clean divergent local and remote histories are merged automatically with hooks and signing disabled. If that merge conflicts, the command preserves the local commit under a local-only recovery ref, aligns the active branch to the stable remote commit, restores the exact existing `.clawpatch` state, and continues. Another active run also makes the command wait, and transient provider, refusal, quota, or timeout failures retry from durable state every 30 seconds until they succeed or the operator interrupts the command. Use `--push none` for verified local commits without publication. Explicit `--fresh` discards the existing ClawPatch queue and rebuilds it while preserving committed ClawPatch configuration; project source must still be clean and no active process may own the repository. `--resume-stopped` remains available only as a compatibility override that forbids an automatic fresh start.
...
| Dirty source no longer matches a stale stopped checkpoint | Preserve every changed source path in a local recovery commit and receipt, restore current HEAD, retire the stale checkpoint, and continue. |
| Pre-existing source remains with no checkpoint owner in a bare external run | Preserve the complete changed tree under a local-only recovery ref and external receipt, restore current HEAD, retain `.clawpatch`, and continue. |
...
- **Stale checkpoint wrappers recover in one command.** When a stopped checkpoint no longer proves the current source changes, the normal command preserves every changed source path under a local-only `refs/clawpatch-supervise/recovery/...` ref, writes an external recovery receipt that separates checkpoint-owned paths from the complete preserved set, restores current HEAD, retires only the stale wrapper, and continues the existing ClawPatch queue. It never pushes the recovery ref.
- **Ownerless source does not create an infinite wait.** If a bare external run finds source changes after checkpoint recovery and no durable checkpoint owns them, it preserves the exact complete changed tree under a local-only recovery ref and external receipt, restores current HEAD, leaves `.clawpatch` untouched, and continues. Strict embedded Manageroo calls still reject pre-existing source instead of changing it.
...
legacy mismatch never grants ownership to changed source; the supervisor instead preserves the
complete changed tree in a recovery ref before retiring the stale wrapper.
For a stopped checkpoint that no longer proves the current dirty tree, all changed source bytes are preserved
