...
- **Python installation cannot inherit host secrets.** Disposable venv creation and every pip installation receive an explicit operational allowlist plus supervisor-owned home, cache, and temporary directories. Host credentials, production variables, package-index settings, SSH agents, and user pip configuration are excluded before repository build code can run.
...
- **Bare invocation owns recovery.** The default command resumes the current `.clawpatch` queue and exact stopped checkpoint, revalidates retained uncertain findings, waits for another active owner, and retries transient failures without exiting. It automatically resets only a proven-clean queue with clean project source; dirty source blocks reset.
