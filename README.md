...
- **Bare invocation owns recovery.** The default command resumes the current `.clawpatch` queue and exact stopped checkpoint, snapshots and clears ambiguous stopped-checkpoint source without data loss, revalidates retained uncertain findings, automatically merges clean divergent Git histories, waits for another active owner, and retries transient failures without exiting. It automatically resets only a proven-clean queue with clean project source.
