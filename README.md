...
- **Finished release work does not strand the queue.** If Git HEAD cleanly advances from a checkpoint's base and its temporary iteration commit still proves the same finding, the supervisor accepts both a retained dangling iteration and an iteration already included in that clean history, then retires only the obsolete recovery wrapper. The same recovery applies when a source-clean checkpoint owns no paths and has no temporary commit, because there is no repair content to lose. It preserves `.clawpatch` and lets ClawPatch select that finding or the next one normally.
...
- **New evidence gets another chance in strict mode.** Revalidation escalates from read-only to workspace-write and then to the authorized trusted host only when ClawPatch reports an environment or sandbox limitation. A definitive code or recorded-validation result returns after one run instead of repeating the same analysis at broader permission levels. The external unattended command records an applied uncertain repair and continues the remaining open queue, matching the documented manual workflow without falsely relabeling the result as fixed.
